    ...
    <main class="mt-[12em] w-100 min-h-screen flex flex-col px-16 gap-4">
        ...
        <div class="flex justify-between fixed top-20 left-0 px-16 w-screen bg-white">
            <div class="flex gap-4 py-8  ">
                ...
                <div class="border rounded-sm max-w-fit flex justify-center flex-row-reverse ">
                    <input
...
<!-- table -->
        ...
        <div class=" flex flex-col gap-6">
            <h2 class="font-semibold flex w-[100%] items-center gap-10">Employees Account <hr class="grow bg-red-100 h-[0.3rem]"></h2>
            <div class="flex gap-4 items-start">
                <table class="table-auto grow">
                    ...
                    <tbody>
                        <tr @click="() => handleRowClick(item)" v-for="item in filteredItemsByName" class=" odd:bg-gray-100 text-gray-500 hover:bg-red-50">
                        ...
                        <td class="p-3 font-semibold" :style="{ color: item.isActive ? 'green' : 'red' }">{{ item.isActive? 'Active': 'Deactivated' }}</td>
                        <td></td>
                        </tr>
                    </tbody>
                </table>
                <div v-if="clickedRow" class="flex flex-col min-w-[340px] max-w-[340px] bg-[#F8F7F7] p-8">
                    <div class="flex p-4">
                    ...
                    <div>
                        <div class="flex flex-col">
                            ...
                            <input type="text" v-model="clickedRow.fName" 
                            class="px-2 py-1 bg-[#F6F6F6] text-[#766E6E] rounded-md border  border-slate-300 placeholder-slate-400 focus:outline-none placeholder:text-slate-400" 
                            />
                        </div>
                        <div class="flex flex-col">
                            <label>Middle</label>
                            <input type="text" v-model="clickedRow.mName" 
                            class="px-2 py-1 bg-[#F6F6F6] text-[#766E6E] rounded-md border  border-slate-300 placeholder-slate-400 focus:outline-none placeholder:text-slate-400" 
                            />
                        </div>
                        <div class="flex flex-col">
                            <label>Last Name</label>
                            <input type="text" v-model="clickedRow.lName" 
                            class="px-2 py-1 bg-[#F6F6F6] text-[#766E6E] rounded-md border  border-slate-300 placeholder-slate-400 focus:outline-none placeholder:text-slate-400" 
                            />
                        </div>
                        <div class="flex flex-col">
                            <label>Email</label>
                            <input type="email" v-model="clickedRow.email" 
                            class="px-2 py-1 bg-[#F6F6F6] text-[#766E6E] rounded-md border  border-slate-300 placeholder-slate-400 focus:outline-none placeholder:text-slate-400" 
                            />
                        </div>
                    </div>
